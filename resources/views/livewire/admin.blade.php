    ...
    <div class="container-fluid">
        <div class="row justify-content-center">
            ...
            <div class=" col-sm-6 col-md-8 mt-4 px-4 text-center">
                <div class="mb-4">
                    <h5 class="fw-bold text-uppercase">Usuarios a Validar</h5>
                </div>

                <div class="row">
                    @foreach($avaliar as $v)
                    <div class="col-md-6 mb-4" wire:key="user-{{ $v->id }}">
                        <div class="card h-100 border-0 shadow-sm hover-shadow transition">
                            <div class="card-body text-center">
                                <h5 class="fw-bold text-dark">{{ $v->name }}</h5>
                                <p class="text-muted small mb-1">{{ $v->email }}</p>

                                <div class="d-flex justify-content-center gap-2 mt-3">
                                    <button wire:click="aceitar({{ $v->id }})"
                                        class="btn btn-success btn-sm rounded-pill px-3">
                                        <i class="bi bi-check-lg"></i> Aceitar
                                    </button>

                                    <button wire:click="recusar({{ $v->id }})"
                                        wire:confirm="Tem certeza que deseja recusar este cadastro?"
